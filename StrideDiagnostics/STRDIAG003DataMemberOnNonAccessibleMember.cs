...
public class PrivateDataMemberAnalyzer : DiagnosticAnalyzer
{
    ...
    private const string Title = "Attribute Contradiction [Stride.Core.DataMember]";
    private const string MessageFormat = "The [Stride.Core.DataMemberAttribute] is invalid on the private Member '{0}'";
    ...
    private static void AnalyzeNode(SymbolAnalysisContext context)
    {
        ...
        if (symbol is IPropertySymbol fieldDeclaration && (
            symbol.DeclaredAccessibility != Accessibility.Public &&
            symbol.DeclaredAccessibility != Accessibility.Internal
            )
        )
        {
            ReportDiagnostics(context, dataMemberAttribute, fieldDeclaration);
        }
    }
