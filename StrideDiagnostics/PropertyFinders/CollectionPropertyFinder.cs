...
internal class CollectionPropertyFinder : IPropertyFinder, IViolationReporter
{
    ...
    public void ReportViolations(ref INamedTypeSymbol baseType, ClassInfo classInfo)
    {
        if (baseType == null)
            return;
        IEnumerable<IPropertySymbol> violations = baseType.GetMembers().OfType<IPropertySymbol>().Where(property => !PropertyHelper.IsArray(property) && !this.ShouldBeIgnored(property) && PropertyHelper.IsICollection_generic(property.Type) && !HasProperAccess(property));
